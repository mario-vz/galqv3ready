...
export const AddPropertyModal = ({ isOpen, onClose, onSuccess }: AddPropertyModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile && property) {
        const imageUrl = await uploadImage(property.id);
        if (imageUrl) {
          await supabase
            .from('properties')
            ...
            <div className="space-y-3">
              <select
                ...
                onChange={(e) => {
                  ...
                  if (value === 'custom') {
                    setFormData({ ...formData, is_custom: true, custom_commission: '' });
                  } else {
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFormData({
                      ...
                      commission_percentage: parseFloat(value) || 0
                    });
                  }}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-atlantic-blue focus:border-transparent"
